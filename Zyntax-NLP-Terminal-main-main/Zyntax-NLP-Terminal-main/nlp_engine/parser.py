...
FUZZY_MATCH_THRESHOLD_SUGGEST = 75
...
ACTION_KEYWORDS = {
    # File/Directory Listing & Navigation (Moved to top for priority)
    ...
    "create new directory": "make_directory",
    "create new folder": "make_directory",
    "make new directory": "make_directory",
    "make new folder": "make_directory",
    "create directory": "make_directory",
    "create folder": "make_directory",
    "make directory": "make_directory",
    "make folder": "make_directory",
    "mkdir": "make_directory",
    "make dir": "make_directory",
    "create dir": "make_directory",
    
    # Navigation & Path
    "show current directory": "show_path",
    "current directory": "show_path",
    "where am i": "show_path",
    "print working directory": "show_path",
    "pwd": "show_path",
...
def extract_relevant_entities(doc, text):
    ...
    filtered_entities = []
    custom_stops = {'file', 'folder', 'directory', 'dir', 'named', 'called', 'with', 'name', 'to', 'from', 'a', 'the', 'in', 'on', 'at', 'make', 'create', 'delete', 'zyntax'}
    ...
    keywords_to_check_fuzzy = ['folder', 'directory', 'delete', 'create', 'file', 'rename', 'move', 'zyntax']
    keywords_to_check_fuzzy.extend(list(action_verbs))

    fuzzy_check_threshold = 90  # Increased threshold for more precise matching

    for entity in potential_entities:
        # Skip if entity is the program name
        if entity.lower() == 'zyntax':
            continue
...
def parse_input(text):
    ...
    # Special handling for file listing commands
    if any(phrase in text_lower for phrase in ["show all files", "list all files", "show files", "list files", "show directory contents", "list directory contents"]):
        return {
            'action': 'list_files',
            'args': []  # No arguments needed for listing current directory
        }

    # Special handling for directory creation commands with "called" or "named"
    if "create" in text_lower and "directory" in text_lower and ("called" in text_lower or "named" in text_lower):
    ...
    # Special handling for directory creation commands with "new"
    if "create" in text_lower and "new" in text_lower and "directory" in text_lower:
        doc = nlp(original_text)
        args = extract_relevant_entities(doc, original_text)
        return {
            'action': 'make_directory',
    ...
    if action_id == 'list_files':
        # No arguments needed for listing current directory
        parsed_args = []
    elif action_id in ['make_directory', 'create_file', 'delete_file', 'delete_directory', 'display_file', 'change_directory'] and args:
        ...
        if msg_match:
            ...
            parsed_args = ["-m", "Generic commit via Zyntax"]
